/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { ArgsType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { OutcomeWhereUniqueInput } from "./OutcomeWhereUniqueInput";
import { ValidateNested } from "class-validator";
import { Type } from "class-transformer";
import { OutcomeUpdateInput } from "./OutcomeUpdateInput";

@ArgsType()
class UpdateOutcomeArgs {
  @ApiProperty({
    required: true,
    type: () => OutcomeWhereUniqueInput,
  })
  @ValidateNested()
  @Type(() => OutcomeWhereUniqueInput)
  @Field(() => OutcomeWhereUniqueInput, { nullable: false })
  where!: OutcomeWhereUniqueInput;

  @ApiProperty({
    required: true,
    type: () => OutcomeUpdateInput,
  })
  @ValidateNested()
  @Type(() => OutcomeUpdateInput)
  @Field(() => OutcomeUpdateInput, { nullable: false })
  data!: OutcomeUpdateInput;
}

export { UpdateOutcomeArgs as UpdateOutcomeArgs };
